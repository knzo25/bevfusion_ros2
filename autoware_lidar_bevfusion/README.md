...
The `autoware_lidar_bevfusion` package is used for 3D object detection based on lidar data (x, y, z, intensity).
...
The `autoware_lidar_bevfusion` node has `build_only` option to build the TensorRT engine file from the ONNX file.
...
- TransFusion: [bevfusion.onnx](https://awf.ml.dev.web.auto/perception/models/bevfusion/t4xx1_90m/v2/bevfusion.onnx)
