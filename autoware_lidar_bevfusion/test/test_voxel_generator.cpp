...
void VoxelGeneratorTest::SetUp()
{
  ...
  voxels_num_ = std::vector<std::int64_t>{5000, 30000, 60000};
  ...
  config_ptr_ = std::make_unique<BEVFusionConfig>(
  ...
  sensor_msgs::PointCloud2Iterator<std::uint8_t> iter_i(*cloud1_, "intensity");

  // Populate the point cloud
  for (std::size_t i = 0; i < modifier.size(); ++i, ++iter_x, ++iter_y, ++iter_z) {
    *iter_x = static_cast<float>(i);
    *iter_y = static_cast<float>(i);
    *iter_z = static_cast<float>(i);
  }
  for (std::size_t i = 0; i < modifier.size(); ++i, ++iter_i) {
    *iter_i = static_cast<std::uint8_t>(i % 256);
  }
