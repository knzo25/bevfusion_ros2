...
template <typename scalar_t, std::size_t MaxDim = TV_MAX_DIM>
struct SimpleVector {
 public:
  ...
  TV_HOST_DEVICE_INLINE std::size_t size() const { return mSize; }
  TV_HOST_DEVICE_INLINE const scalar_t *data() const { return mArray; }
  TV_HOST_DEVICE_INLINE std::size_t empty() const { return mSize == 0; }
...
template <typename scalar_t, std::size_t MaxDim>
...
template <std::size_t MaxDim = TV_MAX_DIM>
struct ShapeBase : public SimpleVector<int, MaxDim> {
  ...
  TV_HOST_DEVICE_INLINE std::size_t size() const {
  ...
  TV_HOST_DEVICE_INLINE std::size_t ndim() const { return this->mSize; }
...
struct ArrayIndexRowMajor<0> {
  TV_HOST_DEVICE_INLINE static unsigned run([[maybe_unused]] const Shape &shape,
                                            [[maybe_unused]] const Shape &indexes) {
    return 0;
  }
};
...
template <>
constexpr const char *simpleTypeName([[maybe_unused]] float val) {
  return "float32";
}
template <>
constexpr const char *simpleTypeName([[maybe_unused]] double val) {
  return "float64";
}
template <>
constexpr const char *simpleTypeName([[maybe_unused]] int val) {
  return "int32";
}
template <>
constexpr const char *simpleTypeName([[maybe_unused]] unsigned val) {
  return "uint32";
}
template <>
constexpr const char *simpleTypeName([[maybe_unused]] long val) {
  return "int64";
}
template <>
constexpr const char *simpleTypeName([[maybe_unused]] unsigned long val) {
  return "uint64";
}
};  // namespace detail
...
struct TensorView {
  ...
  TV_HOST_DEVICE_INLINE TensorView<scalar_t, Rank> &assign(
    ...
    for (std::size_t i = 0; i < size(); ++i) *(ptr++) = *(other_ptr++);
    return *this;
  }
  ...
  TV_HOST_DEVICE_INLINE TensorView<scalar_t, Rank> view(
    ...
    Shape shapes{int(newShapes)...};
    for (std::size_t i = 0; i < shapes.ndim(); ++i) {
      if (shapes[i] == -1) {
        shapes[i] = 1;
  ...
  TV_HOST_DEVICE_INLINE std::size_t size() const { return mShape.size(); }
  ...
  TV_HOST_DEVICE_INLINE TensorView<scalar_t, Rank> subview(
    ...
    std::size_t idxsize = new_shape.ndim();
    for (std::size_t i = idxsize; i < mShape.ndim(); ++i) {
    ...
    #pragma clang loop unroll(enable)
#endif
    for (std::size_t i = sizeof...(Slices); i < mShape.ndim(); ++i) {
 ...
 protected:
  ...
  TV_HOST_DEVICE_INLINE TensorView<scalar_t, Rank> _subview(
    ...
    for (std::size_t i = 0; i < slice_vec.size(); ++i) {
      if (slice_vec[i][1] != -1) {
        new_shape[i] = slice_vec[i][1] - slice_vec[i][0];
        TV_ASSERT(new_shape[i] >= 0);
      } else {
    ...
    auto offset = rowArrayIdx(mShape, start);
    for (std::size_t i = slice_vec.size(); i < mShape.ndim(); ++i) {
      new_shape[i] = mShape[i];
      TV_ASSERT(new_shape[i] >= 0);
    }
    Shape reduced_shape;
...
template <>
constexpr const char *printfTypeFormat([[maybe_unused]] float val) {
  return "%.2f";
}
template <>
constexpr const char *printfTypeFormat([[maybe_unused]] double val) {
  return "%.2f";
}
template <>
constexpr const char *printfTypeFormat([[maybe_unused]] int val) {
  return "%d";
}
template <>
constexpr const char *printfTypeFormat([[maybe_unused]] unsigned val) {
  return "%u";
}
template <>
constexpr const char *printfTypeFormat([[maybe_unused]] long val) {
  return "%ld";
}
template <>
constexpr const char *printfTypeFormat([[maybe_unused]] unsigned long val) {
  return "%lu";
}
};  // namespace detail
