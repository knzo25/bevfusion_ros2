...
class DeviceRegistry<Ret (*)(Args...), f> {
 public:
  ...
  static const int MAX_DEVICE_TYPES =
      std::int8_t(at::DeviceType::COMPILE_TIME_MAX_DEVICE_TYPES);

  void Register(at::DeviceType device, FunctionType function) {
    funcs_[std::int8_t(device)] = function;
  }

  FunctionType Find(at::DeviceType device) const {
    return funcs_[std::int8_t(device)];
  }
 ...
 //private:
  DeviceRegistry() {
    for (std::size_t i = 0; i < MAX_DEVICE_TYPES; ++i) {
      funcs_[i] = nullptr;
    }
  };
...
template <typename T, typename... Args,
          std::enable_if_t<std::is_same<std::decay_t<T>, at::Tensor>::value,
                           bool> = true>
at::Device GetFirstTensorDevice(T&& t, [[maybe_unused]] Args&&... args) {
